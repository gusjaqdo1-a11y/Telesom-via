...
def main_kb(admin=False):
    rows=[['📱 Numbers','💎 VIP Numbers','🌐 Virtual Numbers'],['📲 eSIM','💳 Physical SIM','📡 Data'],['📞 Voice','💬 SMS','🔋 Recharge'],['🌐 Fiber','💰 ZAAD','🏢 Business'],['🏢 Corporate','🔌 IoT','☁️ Cloud'],['🎁 More Services','🎁 Offers','💰 Wallet'],['🛒 My Orders','💵 Payments','👥 Referral'],['🆘 Customer Support','👤 My Profile','🌐 Language']]
...
@router.message(F.text.in_({'📱 Numbers','💎 VIP Numbers','🌐 Virtual Numbers','📲 eSIM','💳 Physical SIM','📡 Data','📞 Voice','💬 SMS','🔋 Recharge','🌐 Fiber','💰 ZAAD','🏢 Business','🏢 Corporate','🔌 IoT','☁️ Cloud','🎁 Offers'}))
async def service_button(m:Message):
    mapping={'📱 Numbers':'numbers','💎 VIP Numbers':'vip','🌐 Virtual Numbers':'virtual','📲 eSIM':'esim','💳 Physical SIM':'sim','📡 Data':'data','📞 Voice':'voice','💬 SMS':'sms','🔋 Recharge':'recharge','🌐 Fiber':'fiber','💰 ZAAD':'zaad','🏢 Business':'business','🏢 Corporate':'corporate','🔌 IoT':'iot','☁️ Cloud':'cloud','🎁 Offers':'offers'}
...
async def order_number(c:CallbackQuery,state:FSMContext):
    ...
    await c.message.answer(f'🛒 <b>Order Created</b>\n\nOrder: <code>{escape(oid)}</code>\nNumber: <code>{safe(x.get("number"))}</code>\nAmount: <b>${money(x.get("price")):.2f}</b>\n\n💳 Send the exact amount using a payment method below, then tap <b>✅ I HAVE PAID</b>.\n\n⚠️ The admin will receive this order only after you confirm that you have paid.',reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='💵 PAY NOW',callback_data=f'choosepay:{oid}')],[InlineKeyboardButton(text='🛒 My Orders',callback_data='myorders'),InlineKeyboardButton(text='🏠 Home',callback_data='home')]]))
    await c.answer()
...
async def catalog_select(c:CallbackQuery):
    ...
    oid=await create_order(c.from_user.id,x.get('service','service'),x.get('title','Package'),x.get('price',0))
    await c.message.answer(f'🛒 <b>Order Created</b>\n\nOrder: <code>{escape(oid)}</code>\nService: {safe(x.get("service"))}\nPackage: {safe(x.get("title"))}\nAmount: <b>${money(x.get("price")):.2f}</b>\n\n💳 Send the exact amount, then confirm your payment below.\n⚠️ Admin receives the order only after payment confirmation.',reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='💵 PAY NOW',callback_data=f'choosepay:{oid}')],[InlineKeyboardButton(text='🛒 My Orders',callback_data='myorders'),InlineKeyboardButton(text='🏠 Home',callback_data='home')]]))
    await c.answer()
...
async def request_service(c:CallbackQuery,state:FSMContext):
    ...
    await state.set_state(UserFlow.order_details)
    await c.message.answer(f'📝 <b>{safe(SERVICES.get(service,service))}</b>\n\nSend the required details. Your request will NOT be sent to admin yet. After the order is created, you must pay and tap <b>✅ I HAVE PAID</b>.')
    await c.answer()

@router.message(UserFlow.order_details)
async def order_details(m:Message,state:FSMContext):
    d=await state.get_data(); oid=await create_order(m.from_user.id,d.get('service'),m.text,0); await state.clear()
    await m.answer(f'🛒 <b>Order Created</b>\n\nOrder: <code>{escape(oid)}</code>\nService: {safe(d.get("service"))}\nDetails: {safe(m.text)}\n\n💳 This request has NOT been sent to admin. Complete payment first, then tap <b>💵 PAY NOW</b>.',reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='💵 PAY NOW',callback_data=f'choosepay:{oid}')],[InlineKeyboardButton(text='🛒 My Orders',callback_data='myorders')],[InlineKeyboardButton(text='🏠 Home',callback_data='home')]]))
...
async def pay_method(c:CallbackQuery,state:FSMContext):
    ...
    text=f'💳 <b>{safe(method)}</b>\n\n📍 Payment destination:\n<code>{escape(str(x.get("destination","")))}</code>\n\n'
    if amount>0:text+=f'💰 Amount: <b>${amount:.2f}</b>\n\n'
    text+='1️⃣ Send the payment using your wallet.\n2️⃣ After you send it, tap <b>✅ I HAVE PAID</b>.\n3️⃣ Enter the payment reference/transaction ID when asked.\n\n⚠️ Do not tap the button before you actually send the money.'
...
async def paid_confirm(c:CallbackQuery,state:FSMContext):
    ...
    await c.message.answer('🔎 <b>Payment Confirmation</b>\n\nSend the transaction/reference ID. If your method does not provide one, send the payer number followed by the amount.\n\nExample: <code>TXN123456</code>')
    await c.answer()
...
async def init_db():
    ...
    website_catalog=[
        ('data','24 Hours Unlimited Data',0.50),('data','7 Days Unlimited Data',3.00),('data','30 Days Unlimited Data',15.00),('data','24 Hours Unlimited Calls + Data',1.00),
        ('data','24 Hours 1 GB',0.25),('data','24 Hours 400 MB',0.12),('data','7 Days 3.5 GB',1.00),('data','7 Days 400 MB',0.12),('data','7 Days 1.8 GB',0.50),('data','7 Days 900 MB',0.25),('data','30 Days 17 GB',5.00),
        ('data','30 Days 8 GB VIP',3.00),('data','30 Days 25 GB VIP',5.00),('data','30 Days 55 GB VIP',10.00),('data','30 Days 100 GB VIP',15.00),('data','30 Days 130 GB VIP',20.00),('data','30 Days 140 GB VIP',25.00),
        ('voice','24 Hours Unlimited Calls + 100 SMS',0.30),('voice','7 Days Unlimited Calls + 500 SMS',2.00),('voice','14 Days Unlimited Calls + 750 SMS',4.00),('voice','30 Days Unlimited Calls + 1000 SMS',7.00),
        ('esim','Telesom eSIM',2.00),('sim','Classic VIP SIM',5.00)
    ]
