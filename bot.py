    ...
    for collection, field, unique in indexes:
        try:
            ...
            if current:
                ...
                # Keep any existing compatible index, regardless of sparse option.
                # This prevents MongoDB IndexKeySpecsConflict when an older deployment
                # already created field_1 without sparse=True.
                if same_key and same_unique:
                    continue
                try:
                    await db[collection].drop_index(name)
                except Exception as drop_error:
                    log.warning("Could not replace index %s.%s: %r", collection, field, drop_error)
                    continue
            try:
                await db[collection].create_index(field, unique=unique, name=name)
            except Exception as create_error:
                # A concurrent/legacy index may have appeared between inspection and creation.
                # Re-check before treating it as fatal.
                latest = await db[collection].index_information()
                if name in latest:
                    log.info("Using existing index %s.%s", collection, field)
                else:
                    raise create_error
        except Exception as e:
